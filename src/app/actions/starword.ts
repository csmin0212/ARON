...
function emptyState(day: string, isGm = false): StarwordState {
  return {
    day,
    isGm,
    started: false,
...
function stateOf(run: {
  ...
  return {
    ...
    gmAnswer: isGm ? run.answer : undefined,
    elapsedMs: run.elapsedMs,
    startedAtMs: run.startedAt.getTime(),
  };
}

function randomStarwordAnswer(except?: string): string {
  if (STARWORD_WORDS.length <= 1) return STARWORD_WORDS[0] ?? "쌍별";
  let answer = STARWORD_WORDS[Math.floor(Math.random() * STARWORD_WORDS.length)];
  while (answer === except) {
    answer = STARWORD_WORDS[Math.floor(Math.random() * STARWORD_WORDS.length)];
  }
  return answer;
}
...
export async function getStarwordState(): Promise<StarwordState> {
  ...
  return run ? stateOf(run, isGm) : emptyState(day, isGm);
}
...
export async function startStarword(): Promise<StarwordState> {
  ...
  if (existing) return stateOf(existing, isGm);

  const sheet = await prisma.characterSheet.findUnique({
    where: { userId: user.id },
    select: { curGold: true, locationId: true },
  });
  if (!sheet) return { ...emptyState(day, isGm), error: "캐릭터 시트 연동이 필요합니다." };
  ...
  try {
    ...
    return again ? stateOf(again, isGm) : { ...emptyState(day, isGm), error: "이미 오늘 판을 시작했어요." };
  }
  ...
  return stateOf(run, isGm);
}

/** GM 테스트 — 오늘 내 판의 단어를 새 단어로 바꾸고 시도를 초기화한다. */
export async function replaceStarwordForGm(): Promise<StarwordState> {
  const day = starwordDayKey();
  const user = await getCurrentUser();
  if (!user) return { ...emptyState(day), error: "로그인이 필요합니다." };
  if (!isGmUsername(user.username)) return { ...emptyState(day), error: "GM 권한이 필요합니다." };

  const existing = await prisma.starwordRun.findUnique({
    where: { userId_day: { userId: user.id, day } },
  });
  const answer = randomStarwordAnswer(existing?.answer);
  const now = new Date();
  const run = existing
    ? await prisma.starwordRun.update({
    ...
    : await prisma.starwordRun.create({
        data: { userId: user.id, day, answer, startedAt: now },
      });

  revalidatePath("/world");
  return stateOf(run, true);
}
...
export async function guessStarword(raw: string): Promise<StarwordState> {
  ...
  if (!run) return { ...emptyState(day, isGm), error: "먼저 입장해주세요." };
  if (run.clearedAt || run.failed) return stateOf(run, isGm);

  const word = raw.trim();
  if (!isValidStarwordShape(word)) {
    return { ...stateOf(run, isGm), error: "두 글자 한글 단어를 입력해주세요." };
  }
  if (!isKnownStarword(word)) {
    return { ...stateOf(run, isGm), error: "목록에 없는 단어예요." };
  }

  const guesses = parseGuesses(run.guessJson);
  if (guesses.includes(word)) return { ...stateOf(run, isGm), error: "이미 시도한 단어예요." };
  guesses.push(word);
...
export async function getStarwordRanking(day?: string): Promise<StarwordRankRow[]> {
  ...
  return rows
    .filter((r) => !isGmUsername(r.user.username))
    .slice(0, 10)
    .map((r) => ({
...
export async function settleStarwordRanks(): Promise<void> {
  ...
  for (const { day } of pending) {
    ...
    const rewardTargets = ranked.filter((run) => !isGmUsername(run.user.username));
    for (const [index, run] of rewardTargets.entries()) {
