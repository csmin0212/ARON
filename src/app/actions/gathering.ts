...
async function grant(userId: string, nickname: string, locationId: string | null, p: Pending, how: string) {
  ...
  const expBase = lifeSkillExpGain(GATHER, p.exp);
  ...
  if (originLocationId) {
    const actor = { userId, actorName: nickname, kind: "채집" as const };
    await postSystem(
      originLocationId,
      `🌿 ${nickname}님 ${how} — [${p.rarity}] ${p.name} x1 (판매가 ${sell}G) +숙련도 ${expText}${
    ...
    for (const lv of leveled) {
      ...
      await postSystem(originLocationId, `🆙 ${nickname}님의 채집 레벨이 ${lv}이 되었다!${perkPrompt}`, actor);
    }
  }
...
export async function startGathering(): Promise<GatherStart> {
  ...
  const gatherToolTier = toolTier(life.tools.채집);
  mods.luck += toolRankRateBonus(gatherToolTier, mods.toolEff);
  ...
  const rodRelief = gatherToolTier * 10 * (1 + mods.toolEff / 100);
