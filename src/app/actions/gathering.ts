...
async function ensureItem(p: Pending): Promise<void> {
  ...
  await prisma.item.upsert({
    where: { id: p.name },
    create: { id: p.name, name: p.name, category: lifeSkillCategory(GATHER), sellPrice, desc: p.text, order: p.no },
    update: { name: p.name, category: lifeSkillCategory(GATHER), sellPrice, desc: p.text, order: p.no },
  });
}
...
async function grant(userId: string, nickname: string, locationId: string | null, p: Pending, how: string) {
  ...
  if (lifeBagWeight(bag) + p.weight > lifeBagLimit(life, GATHER, mods.weightBonus)) {
...
export async function startGathering(): Promise<GatherStart> {
  ...
  const bag = life.bags[GATHER];
  if (lifeBagWeight(bag) + item.weight > lifeBagLimit(life, GATHER, mods.weightBonus)) {
    return { error: `${bag.name}이 가득 찼어요.` };
  }

  const rankDiff = RANK_DIFFICULTY[item.rank] ?? 60;
  const rodRelief = toolTier(life.tools.채집) * 10;
  ...
  await prisma.characterSheet.update({
    ...
    data: { ap: ap - action.apCost, apResetAt, pendingGatherJson: JSON.stringify(pending) },
  });
