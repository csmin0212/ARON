...
export default function WorldServices({ canShop, canForge, shopItems, inventoryItems }: Props) {
  const [open, setOpen] = useState<"shop" | "forge" | null>(null);
  const [forgeTab, setForgeTab] = useState<"weapon" | "magic">("weapon");
  const [shopState, shopAction, shopPending] = useActionState<ServiceState, FormData>(
    buyShopItem,
    undefined,
  );
  ...
  if (!canShop && !canForge) return null;
        ...
        <div className="grid grid-cols-2 gap-2">
          {canShop && (
            <button
              ...
              className="rounded-2xl border border-line bg-subtle px-3 py-3 text-left transition hover:border-brand-300 hover:bg-brand-50"
            >
              <span className="block text-lg">🛒</span>
              <span className="mt-1 block text-sm font-extrabold text-content">상점</span>
              <span className="text-[11px] text-faint">아이템 구매</span>
            </button>
          )}
          {canForge && (
            <button
              type="button"
              onClick={() => setOpen("forge")}
              className="rounded-2xl border border-line bg-subtle px-3 py-3 text-left transition hover:border-brand-300 hover:bg-brand-50"
            >
              <span className="block text-lg">⚒️</span>
              <span className="mt-1 block text-sm font-extrabold text-content">대장간</span>
              <span className="text-[11px] text-faint">무기 강화</span>
            </button>
          )}
        </div>
      </div>
        ...
        >
          ...
          >
            <div className="flex items-center justify-between border-b border-line bg-subtle px-5 py-4">
              <div>
                <p className="text-[11px] font-bold uppercase tracking-wide text-faint">
                  {open === "shop" ? "Shop" : "Forge"}
                </p>
                <h3 className="mt-1 text-xl font-extrabold text-content">
                  {open === "shop" ? "상점" : "대장간"}
                </h3>
              </div>
              <button
            ...
            <div className="max-h-[70vh] overflow-y-auto px-5 py-4">
              {open === "shop" && (
                <form action={shopAction} className="space-y-3">
                  <StateLine state={shopState} />
                  <label className="block">
                    <span className="mb-1 block text-xs font-bold text-faint">상품</span>
                    <select
                      name="itemId"
                    ...
                    >
                      {shopItems.map((item) => (
                        <option key={item.id} value={item.id}>
                          {item.name} - {item.buyPrice?.toLocaleString("ko-KR")}G
                        </option>
                      ))}
                    </select>
                  </label>
                  <label className="block">
                    <span className="mb-1 block text-xs font-bold text-faint">수량</span>
                    <input
                      name="qty"
                      type="number"
                      min={1}
                  ...
                  {shopItems.length === 0 && (
                    <p className="rounded-xl bg-subtle px-3 py-3 text-sm text-faint">
                      구매 가능한 상품이 아직 없어요. 아이템 시트의 구매가를 채우면 여기에
                      나타납니다.
                    </p>
                  )}
                  <button
                    type="submit"
                    disabled={shopPending || shopItems.length === 0}
                ...
                <div className="space-y-4">
                  ...
                  {forgeTab === "weapon" ? (
                    <form action={upgradeAction} className="space-y-3">
                      ...
                      {weapons.length === 0 && (
                        <p className="rounded-xl bg-subtle px-3 py-3 text-sm text-faint">
                          인벤토리에서 무기 후보를 찾지 못했어요. 장비 중인 무기는 시트 휴대품에
                          넣은 뒤 동기화해주세요.
                        </p>
                      )}
                      <button
