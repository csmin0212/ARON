  ...
  if (post.author) {
    return (
      <span className="flex items-center gap-1.5">
        ...
        <span className="truncate font-medium text-slate-600">{post.author.nickname}</span>
      </span>
    );
  }
  return (
    <span className="flex items-center gap-1.5 text-slate-400">
      ...
      <span className="truncate">
        {post.anonNick ?? "ㅇㅇ"}
        <span className="ml-0.5 text-[11px] text-slate-300">({post.anonIp})</span>
      </span>
    </span>
  );
}
...
function Row({ post }: { post: ListPost }) {
  return (
    <li className={post.pinned ? "bg-brand-50/60" : "transition hover:bg-slate-50/80"}>
      <Link href={`/post/${post.id}`} className="flex items-center gap-3 px-3 py-2.5 sm:px-4">
        {/* 번호 / 핀 */}
        <span className="hidden w-10 shrink-0 text-center text-xs text-slate-300 sm:block">
          {post.pinned ? "📌" : post.id}
        </span>
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1.5">
            <span className="truncate text-[15px] font-medium text-slate-800">{post.title}</span>
          ...
          <div className="mt-1 flex items-center gap-2 text-[11px] text-slate-400 sm:hidden">
        ...
        <span className="hidden w-12 shrink-0 text-center text-xs text-slate-400 sm:block">
        ...
        <span
          className={`hidden w-12 shrink-0 text-center text-xs font-semibold sm:block ${
            post.recommendCount > 0 ? "text-rose-500" : "text-slate-300"
          }`}
        >
          {post.recommendCount}
        </span>
...
export default function PostList({ posts }: { posts: ListPost[] }) {
  return (
    <div className="overflow-hidden rounded-2xl border border-line bg-white shadow-sm">
      {/* 헤더 (데스크탑) */}
      <div className="hidden items-center gap-3 border-b border-line bg-slate-50/70 px-4 py-2.5 text-[11px] font-bold text-slate-400 sm:flex">
